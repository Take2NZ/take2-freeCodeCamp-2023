...
Inside the `footer`, add a `p` element. Then, nest an anchor (`a`) element in the `p` that links to `http://localhost:8000` and has the text `Visit our website`.
...
Your new `a` element should link to `http://localhost:8000`. Remember that `a` elements use the `href` attribute to create a link.

```js
assert(document.querySelector("footer > p > a")?.href === "http://localhost:8000/");
```

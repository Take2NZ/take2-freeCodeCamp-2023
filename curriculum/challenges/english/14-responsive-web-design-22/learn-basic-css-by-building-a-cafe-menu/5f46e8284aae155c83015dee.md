...
Under the `Coffee` heading, add an image using the url `http://localhost:8000/images/css-cafe/coffee.jpg`. Give the image an `alt` value of `coffee icon`.
...
Your `img` element should have a `src` attribute of `http://localhost:8000/images/css-cafe/coffee.jpg`.

```js
assert($('img').attr('src') === 'http://localhost:8000/images/css-cafe/coffee.jpg');
```

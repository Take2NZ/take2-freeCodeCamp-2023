...
`http://localhost:8000/static/cat-photo-app/lasagna.jpg`
...
The new image should have an `src` value of `http://localhost:8000/static/cat-photo-app/lasagna.jpg`. Make sure the `src` attribute's value is surrounded with quotation marks.
...
```js
assert(!/\<img\s+.+\s+src\s*=\s*http:\/\/localhost\:8000\/static\/cat-photo-app\/lasagna\.jpg/.test(code));
```

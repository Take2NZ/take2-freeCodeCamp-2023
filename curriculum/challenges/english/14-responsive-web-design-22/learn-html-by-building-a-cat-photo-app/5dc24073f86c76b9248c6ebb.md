...
Your `img` element's `src` attribute should be set to '`http://localhost:8000/static/cat-photo-app/relaxing-cat.jpg`'. You have either omitted the URL or have a typo. The case of the URL is important.

```js
assert(document.querySelector('img').src === 'http://localhost:8000/static/cat-photo-app/relaxing-cat.jpg');
```
...
```js
assert(!/\<img\s+src\s*=\s*http:\/\/localhost\:8000\/static\/cat-photo-app\/relaxing-cat\.jpg/.test(code));
```
